 ...
 * <p>
 * <h2>调度</h2>
 * 复合执行器中至少有一组线程用于任务的调度，这组线程称之为：<b>调度线程</b>。调度线程可以在本地直接执行任务，
 * 也可以将任务派发给其它执行器执行。调度线程可以同时存在多组，多组调度线程联合工作，构成：<b>多重调度</b>。
 * 多重调度可以缓解某个调度线程的压力。
 * <p>
 ...
 * <p>
 * <h2>复合执行</h2>
 * 复合执行器是为业务分层执行而设计的，例如：密集型计算、远程RPC调用、读写文件等涉及IO的操作。在处理非CPU密集型任务时，
 * 同步等待会占用当前线程，异步回调又使得代码晦涩难懂。当使用复合执行器时，只需要将任务分层，按照执行时长划分，
 * 不同的执行时长采用不同的执行器。
 * <p>
 * 调度线程参与执行任务时，在极限情况下，所有任务可能堆积到某个线程，此时还可以使用复合执行器缓解调度线程压力。
 * 复合执行器在一定意义上支持了任务的窃取，当调度线程过于繁忙或堆积大量任务时，可以将一些任务派遣至其它执行器中执行。
 * 此外，复合执行器同样可以用于限流，将某个资源的访问派遣到某个特定的执行器，在执行器中进行限流。
 * <p>
 * <h2>协程</h2>
 * 复合执行器支持使用协程。复合执行器的设计，在一定意义上也是为了弥补协程缺失所带来的问题（但不仅仅是）。遗憾的是，
 * 受限于无法控制并发协程数量，所以，并没有直接支持协程，仅仅是预留部分接口，开发人员可通过实现{@link Handler}
 * 接口支持协程的使用。
 * <p>
 * <b>注意</b>：可以将非CPU密集型的任务交由协程处理，但需要开发人员自行预估并发协程数量，防止过多的协程耗尽内存。
 * CPU密集型的长耗时任务，同样不建议交由协程处理，而是应该交由真实线程执行，真实线程被操作系统按照时间片调度，
 * 这才是更合理的方案。
 * <p>
 * <h2>使用</h2>
 * 演示将任务按照不同的执行时长划分，使用不同的执行器：
 * <pre>{@code
 * enum HandlerType {
 *     LOCAL, LOW, MID, HIGH
 * }
...
public interface CompositeHandler<E extends Enum<E>> extends Dispatcher<Runnable> {
	 ...
	 * 调度一个可执行任务
	 * <p>
	 * 调度一个任务到复合执行器中，并在将来的某个时间执行给定的任务。根据执行器的实现，
